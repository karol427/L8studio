...
import { Plus, X, Save, ChevronLeft, Check, Upload, Loader } from 'lucide-react'
...
export default function EventForm({ event = null, onSave }) {
  ...
  const { add: addEvent } = useCollection('wydarzenia')
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await addEvent({ ...form, schedule, status: 'Niepotwierdzony' })
      navigate('/wydarzenia')
    } catch(err) {
